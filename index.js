...
function showServiceStep() {
  ...
  document.getElementById('booking-slots-grid').innerHTML = bookingServicesList.map(s =>
    '<button class="booking-slot-btn" style="grid-column:1/-1;text-align:left;display:flex;justify-content:space-between;align-items:center" data-act="choose-service" data-arg="' + s.id + '"><span>' + s.name + ' · ' + s.duration_minutes + ' min</span>' + (s.price ? '<span style="color:#3b82f6">' + s.price + '€</span>' : '') + '</button>'
  ).join('');
}
...
function showStaffStep() {
  ...
  document.getElementById('booking-slots-grid').innerHTML = bookingStaffList.map(s =>
    '<button class="booking-slot-btn" style="grid-column:1/-1;text-align:left" data-act="choose-staff" data-arg="' + s.id + '">👤 ' + s.name + '</button>'
  ).join('');
}
