...
async function createWalletPass(customer) {
  ...
  const { goal: effectiveGoal, reward: effectiveReward, isFinal } = getEffectiveGoal(customer);
  const remaining = Math.max(0, effectiveGoal - (customer.points || 0));
  const rewardLabel = isFinal ? 'Récompense' : 'Prochain palier';
  ...
  if (tiersText) {
    textModulesData.push({ id: 'tiers', header: 'Tous les paliers', body: tiersText });
  }
  ...
  const loyaltyObject = {
    ...
    loyaltyPoints: {
      label: 'Points sur ' + effectiveGoal,
...
// Si des paliers de récompense sont configurés, le recto du pass Wallet (barre de progression,
// objectif, récompense) cible le PROCHAIN palier non débloqué — pas l'objectif final — pour
// mettre en avant la prochaine petite victoire accessible plutôt qu'un objectif lointain.
// Une fois tous les paliers débloqués, affiche le dernier comme état "complet".
function getEffectiveGoal(customer) {
  const tiers = customer.reward_tiers || [];
  if (tiers.length > 0) {
    const points = customer.points || 0;
    const nextTier = tiers.find(t => points < t.threshold_points);
    if (nextTier) {
      // Progression vers le prochain palier proche (pas encore débloqué) — plus motivant à court terme
      return { goal: nextTier.threshold_points, reward: nextTier.reward_text, isFinal: false };
    }
    // Tous les paliers sont débloqués : on affiche le dernier (le plus haut) comme état "complet"
    const top = tiers[tiers.length - 1];
    return { goal: top.threshold_points, reward: top.reward_text, isFinal: true };
  }
  return { goal: customer.points_goal || 0, reward: customer.reward_text || 'Non définie', isFinal: true };
}

// Texte lisible listant chaque palier et s'il est débloqué — affiché au dos du pass Apple
// et en module texte sur le pass Google, pour que les paliers soient visibles dans le Wallet.
function tiersSummaryText(customer) {
  const tiers = customer.reward_tiers || [];
  if (tiers.length === 0) return null;
  return tiers.map(t => (customer.points >= t.threshold_points ? '✓ ' : '○ ') + t.threshold_points + ' pts : ' + t.reward_text).join('\n');
}
...
async function createApplePassBuffer(customer) {
  ...
  const { goal: effectiveGoal, reward: effectiveReward, isFinal } = getEffectiveGoal(customer);
  const remaining = Math.max(0, effectiveGoal - (customer.points || 0));
  const goalHeaderLabel = isFinal ? 'OBJECTIF' : 'PROCHAIN PALIER';
  const rewardBackLabel = isFinal ? 'Récompense' : 'Prochain palier';

  const backFields = [
    { key: 'reward', label: rewardBackLabel, value: effectiveReward },
  ...
  const tiersText = tiersSummaryText(customer);
  if (tiersText) {
    backFields.splice(1, 0, { key: 'tiers', label: 'Tous les paliers', value: tiersText });
  }
  if (customer.menu_url) {
  ...
  const passJson = {
    ...
    storeCard: {
      ...
      primaryFields: [
        { key: 'points', label: 'POINTS', value: customer.points + ' / ' + effectiveGoal + ' pts' }
      ],
      secondaryFields: [
      ...
      auxiliaryFields: [
        ...
        { key: 'progress', label: 'PROGRESSION', value: progressBarText(customer.points, effectiveGoal) }
      ],
      backFields
    },
    barcodes: [{
